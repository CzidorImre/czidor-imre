/* ───────────────────────────────────────────────────────────
   CZIDOR IMRE — Scroll-built modern family house (Three.js r128)
   Pieces are crane-placed: they settle down + fade into position
   as you scroll. Dusk lighting, warm windows, petrol-teal accent.
   API: window.HouseScene.setProgress(p)  (0 → empty plot, 1 → done)
   ─────────────────────────────────────────────────────────── */
(function () {
  function boot() {
    ...
    // ── Scene + fog (blends into the dark page) ──
    ...
    // ── Controls (only after the house is complete) ──
    ...
    scene.add(new THREE.HemisphereLight(0xfdfbf6, 0x6b6256, 0.9));
    ...
    scene.add(sun);
    const moon = new THREE.DirectionalLight(0x7aa7d6, 0.4);
    moon.position.set(-11, 8, -7); scene.add(moon);
    const warm = new THREE.PointLight(0xffb061, 0, 18, 2); // interior glow, ramps in with glazing
    warm.position.set(1.2, 2.4, 3.0); scene.add(warm);

    // ── Materials ──
    const M = {
      wall:    new THREE.MeshStandardMaterial({ color: 0xeae4d8, roughness: 0.95 }),
      dark:    new THREE.MeshStandardMaterial({ color: 0x2b2d31, roughness: 0.65, metalness: 0.12 }),
      wood:    new THREE.MeshStandardMaterial({ color: 0xa9743e, roughness: 0.8 }),
      concrete:new THREE.MeshStandardMaterial({ color: 0xc7c1b3, roughness: 0.96 }),
      rebar:   new THREE.MeshStandardMaterial({ color: 0x8a8f97, roughness: 0.5, metalness: 0.6 }),
      glass:   new THREE.MeshStandardMaterial({ color: 0x9fc4d6, roughness: 0.06, metalness: 0.25, transparent: true, opacity: 0.42 }),
      glassWarm:new THREE.MeshStandardMaterial({ color: 0x33373d, roughness: 0.1, metalness: 0.2, emissive: 0x3E8A95, emissiveIntensity: 0.12 }),
      water:   new THREE.MeshStandardMaterial({ color: 0x3a8f97, roughness: 0.1, metalness: 0.35, transparent: true, opacity: 0.82, emissive: 0x16494e, emissiveIntensity: 0.35 }),
      deck:    new THREE.MeshStandardMaterial({ color: 0x8c6239, roughness: 0.88 }),
      accent:  new THREE.MeshStandardMaterial({ color: 0x3E8A95, roughness: 0.45, emissive: 0x3E8A95, emissiveIntensity: 0.14 }),
      trunk:   new THREE.MeshStandardMaterial({ color: 0x5a4632, roughness: 1 }),
      leaf:    new THREE.MeshStandardMaterial({ color: 0x6f7d56, roughness: 1 }),
      leaf2:   new THREE.MeshStandardMaterial({ color: 0x5d6b48, roughness: 1 }),
      grass:   new THREE.MeshStandardMaterial({ color: 0xDED8CC, roughness: 1 }),
      soil:    new THREE.MeshStandardMaterial({ color: 0xC9C1B1, roughness: 1 }),
    };
    ...
    function applyBuild(p) {
      for (const b of builders) {
        ...
        if (b.mode === 'pop') {
          ...
          const e = easeOut(t);
          b.obj.position.y = b.targetY + (1 - e) * b.drop;     // settle down into place
          const s = 0.97 + 0.03 * e;                           // tiny settle scale
      ...
      warm.intensity = Math.max(0, Math.min(1, (p - 0.26) / 0.12)) * 0.45;
    }

    // ════════ SITE (static plot — present from the start) ════════
    ...
    // ── Camera path (scroll-driven during build) ──
    ...
    const obs = new IntersectionObserver((ents) => {
      ents.forEach(e => { if (e.isIntersecting) { if (!raf) animate(); } else { cancelAnimationFrame(raf); raf = null; } });
    }, { threshold: 0.01 });
    obs.observe(mount);
